...
def crawling(urls, options): #urls: 리스트
    ...
    for url in urls:
        ...
        #check IP (tor)
        try:
            ipcheck = 'http://icanhazip.com/'
            driver.get(ipcheck)
            html = driver.page_source
            soup = BeautifulSoup(html, 'html.parser')
            text = soup.select_one("body > pre").get_text()
            print(text)
        except:
            pass
...
if __name__ == "__main__":
    ...
    print("-----------Start crawl_parallel-----------")
    
    log.info("crawler setting")
    all_urls = sys.argv[1]
    #print(type(all_urls))//str
    log.info(f'{all_urls}')
    urls = all_urls.split(",")
    urls[-1]=urls[-1].strip('\r') #개행문자 제거
    log.info(f'{urls}')
    data = crawling(urls, options)
